...
Given /^a user named "([^"]*)"$/ do |name|
  User.create(name:name,email: name+"@example.com",password: "password",uid: name )
end
Given /^"([^"]*)" is a follower of "([^"]*)"$/ do |name1,name2|
  follower=User.where(name: name1).first
  followed=User.where(name: name2).first
  Follow.create(follower: follower,followed_user: followed )
end
Given('I see {string} user page') do |string|
...
Then("{string} is on the chat page with {string}") do |name1,name2|
  ...
  page_name=Chat.where(user1:user1,user2:user2).first
  current_path = URI.parse(current_url).path
  if current_path.respond_to? :should
    current_path.should == chat_path(page_name)
  else
    assert_equal chat_path(page_name), current_path
  end
end
